        ...
        try {
            ...
            if (firebaseUser) {
                ...
                if (profileRole === 'manager' || profileRole === 'admin') {
                    setError('This account belongs to the Management Platform. Please sign in there.');
                    setLoading(false);
                    return;
                }
        ...
        <div style={{ minHeight: '100vh', display: 'flex', background: '#f8fafc', fontFamily: 'var(--font-heading)' }}>
            ...
            <div style={{ flex: '1.2', background: 'var(--secondary)', color: 'white', padding: '6rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ zIndex: 10, display: 'flex', gap: '3rem', opacity: 0.6 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        <span style={{ fontSize: '0.85rem', fontWeight: 800, letterSpacing: '1px' }}>SECURE TRADING</span>
                    </div>
                </div>
            </div>
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '5rem', background: 'white' }}>
                <div style={{ width: '100%', maxWidth: '400px' }}>
                    <Link href="/trade/signup" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', color: 'var(--text-soft)', textDecoration: 'none', fontSize: '0.9rem', fontWeight: 800, marginBottom: '4rem', padding: '0.5rem 1rem', background: '#f1f5f9', borderRadius: '10px' }}>
                        <ChevronLeft size={18} /> CREATE ACCOUNT
                    </Link>

                    <div className="fade-up">
                        <h2 style={{ fontSize: '3rem', fontWeight: 900, marginBottom: '0.75rem', letterSpacing: '-0.04em' }}>Sign In</h2>
                        <p style={{ color: 'var(--text-muted)', marginBottom: '3.5rem', fontSize: '1.15rem', fontWeight: 500 }}>Enter your details to access the Trade Platform.</p>
                    ...
                    <p style={{ textAlign: 'center', marginTop: '4rem', color: 'var(--text-muted)', fontSize: '1rem', fontWeight: 600 }}>
                        Manager or Admin? <Link href="/system/login" style={{ color: 'var(--primary)', textDecoration: 'none', fontWeight: 900, borderBottom: '2px solid var(--primary-soft)' }}>System Login</Link>
                    </p>
                </div>
            </div>
        </div>

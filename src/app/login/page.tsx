...
export default function InstitutionalLogin() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (authError) {
                setError(authError.message === 'Invalid login credentials' ? 'Terminal Access Denied: Verification failed.' : authError.message);
                setLoading(false);
                return;
            }
            ...
            if (isMounted.current) {
                ...
                setError('Handshake synchronization failed.');
                setLoading(false);
            }
        }
    };
            ...
            <div style={{ flex: '1.2', background: 'var(--secondary)', color: 'white', padding: '6rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative', overflow: 'hidden' }}>
                ...
                <div style={{ zIndex: 10, position: 'relative' }}>
                    <h1 style={{ fontSize: '5rem', fontWeight: 900, lineHeight: 0.95, marginBottom: '2.5rem', letterSpacing: '-0.05em' }}>
                        Operational <br /><span style={{ color: 'var(--primary)' }}>Logistics Hub.</span>
                    </h1>
                    <p style={{ fontSize: '1.2rem', opacity: 0.6, maxWidth: '440px', lineHeight: 1.6 }}>
                        Authorized terminal access for regional warehouse cluster administrators and verified network buyers.
                    </p>
                </div>
                ...
                <div style={{ zIndex: 10, display: 'flex', gap: '3rem', opacity: 0.6 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                        ...
                        <span style={{ fontSize: '0.85rem', fontWeight: 800, letterSpacing: '1px' }}>FIPS 140-2 ENCRYPTED</span>
                    </div>
                </div>
            </div>
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '5rem', background: 'white' }}>
                <div style={{ width: '100%', maxWidth: '400px' }}>
                    ...
                    <div className="fade-up">
                        ...
                        <p style={{ color: 'var(--text-muted)', marginBottom: '3.5rem', fontSize: '1.15rem', fontWeight: 500 }}>Secure session initialization required.</p>

                        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                <div>
                                    <label style={{ display: 'block', marginBottom: '0.75rem', fontWeight: 800, fontSize: '0.75rem', color: 'var(--secondary)', letterSpacing: '1.5px', textTransform: 'uppercase' }}>Terminal Email</label>
                                ...
                                <div>
                                    <label style={{ display: 'block', marginBottom: '0.75rem', fontWeight: 800, fontSize: '0.75rem', color: 'var(--secondary)', letterSpacing: '1.5px', textTransform: 'uppercase' }}>Access Code</label>
                            ...
                            <button type="submit" className="btn-modern btn-primary-modern" style={{ height: '72px', borderRadius: '20px', fontSize: '1.2rem', boxShadow: '0 10px 20px rgba(5, 150, 105, 0.2)' }} disabled={loading}>
                                {loading ? <Loader2 className="animate-spin" size={28} /> : <>Initialize Terminal <ArrowRight size={24} /></>}
                            </button>
                        </form>
                    </div>

                    <p style={{ textAlign: 'center', marginTop: '4rem', color: 'var(--text-muted)', fontSize: '1rem', fontWeight: 600 }}>
                        Need an institutional node? <Link href="/signup" style={{ color: 'var(--primary)', textDecoration: 'none', fontWeight: 900, borderBottom: '2px solid var(--primary-soft)' }}>Request Enrollment</Link>
                    </p>
                </div>
            </div>
        </div>
